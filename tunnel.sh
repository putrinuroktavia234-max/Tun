...
#================================================
# ── BOX DRAWING (RESPONSIVE & COLOR-AWARE) ──
#================================================

# Hitung panjang teks tanpa warna
str_len() {
    local s
...
# Lebar terminal otomatis (min 55, max 75)
get_width() {
    local tw
...
BASHEOF
    ...
    # Matikan broadcast journald ke terminal
...
show_info_port() {
    ...
    local inner=$(( W - 4 ))
    printf "  ${CYAN}┌"; printf '─%.0s' $(seq 1 $inner); printf "┐${NC}\n"
    printf "  ${CYAN}│${NC}  ${WHITE}%-25s${NC} : ${GREEN}%-$((inner-30))s${NC}  ${CYAN}│${NC}\n" "SSH"               "22"
    printf "  ${CYAN}│${NC}  ${WHITE}%-25s${NC} : ${GREEN}%-$((inner-30))s${NC}  ${CYAN}│${NC}\n" "Dropbear"          "222"
    printf "  ${CYAN}│${NC}  ${WHITE}%-25s${NC} : ${GREEN}%-$((inner-30))s${NC}  ${CYAN}│${NC}\n" "Nginx NonTLS"      "80"
    printf "  ${CYAN}│${NC}  ${WHITE}%-25s${NC} : ${GREEN}%-$((inner-30))s${NC}  ${CYAN}│${NC}\n" "Nginx Download"    "81"
...
def on_info(msg):
    chat_id = msg['chat']['id']
    ip = get_ip()
    send(chat_id, f'ℹ️ <b>INFO SERVER</b>\n🌐 Domain : <code>{DOMAIN}</code>\n🖥️ IP VPS : <code>{ip}</code>\n🔌 SSH: 22 | Dropbear: 222\n🔌 TLS: 443 | NonTLS: 80 | gRPC: 8444', markup=kb_main())
...
menu_advanced() {
    while true; do
        ...
        _box_divider $W
        _box_two $W "[1]  Port Management" "[7]  Firewall Rules"
        _box_two $W "[2]  Protocol Settings" "[8]  Bandwidth Monitor"
        _box_two $W "[3]  Auto Backup" "[9]  User IP Limits"
        _box_two $W "[4]  SSH Brute Protect" "[10] Custom Payload"
        _box_two $W "[5]  Fail2Ban Setup" "[11] Cron Jobs"
        _box_two $W "[6]  DDoS Protection" "[12] System Logs"
        _box_divider $W
...
_adv_protocol_settings() {
    ...
    if [[ -f "$XRAY_CONFIG" ]]; then
        ...
        echo -e "  Total Inbounds: ${GREEN}${inbound_count:-0}${NC}"
        jq -r '.inbounds[] | "  → \(.tag) | port \(.port) | \(.protocol)"' "$XRAY_CONFIG" 2>/dev/null
    fi
    echo ""
    echo -e "  ${WHITE}[1]${NC} Restart Xray  ${WHITE}[2]${NC} Lihat Config  ${WHITE}[0]${NC} Back"
    read -p "  Select: " c
    case $c in
        1) systemctl restart xray && echo -e "  ${GREEN}✔ Restarted!${NC}"; sleep 2 ;;
        2) clear; cat "$XRAY_CONFIG" 2>/dev/null; echo ""; read -p "  Press any key..." ;;
    esac
}

_adv_auto_backup() {
    clear; print_menu_header "AUTO BACKUP CONFIG"
    echo -e "  ${WHITE}[1]${NC} Enable Auto Backup  ${WHITE}[2]${NC} Disable  ${WHITE}[3]${NC} Backup Sekarang  ${WHITE}[0]${NC} Back"
    read -p "  Select: " c
    case $c in
        1)
        ...
        2) crontab -l 2>/dev/null | grep -v "vpn-backup" | crontab -; echo -e "  ${YELLOW}Dimatikan.${NC}"; sleep 2 ;;
        3) _menu_backup ;;
    esac
}

_adv_ssh_brute_protection() {
    clear; print_menu_header "SSH BRUTE FORCE PROTECTION"
    echo -e "  ${WHITE}[1]${NC} Aktifkan  ${WHITE}[2]${NC} Lihat Block  ${WHITE}[3]${NC} Reset  ${WHITE}[0]${NC} Back"
    read -p "  Select: " c
    case $c in
        1)
        ...
        2) clear; iptables -L INPUT -n 2>/dev/null | grep "DROP\|REJECT" | head -20; echo ""; read -p "  Press any key..." ;;
        3) iptables -F 2>/dev/null; echo -e "  ${GREEN}✔ Reset!${NC}"; sleep 2 ;;
    esac
}
...
_adv_ddos_protection() {
    ...
    echo -e "  ${WHITE}[1]${NC} Aktifkan  ${WHITE}[2]${NC} Lihat Statistik  ${WHITE}[3]${NC} Reset  ${WHITE}[0]${NC} Back"
    read -p "  Select: " c
    case $c in
        1)
            sysctl -w net.ipv4.tcp_syncookies=1 >/dev/null 2>&1
            iptables -A INPUT -p tcp ! --syn -m state --state NEW -j DROP 2>/dev/null
            iptables -A INPUT -p tcp --dport 443 -m connlimit --connlimit-above 80 -j REJECT 2>/dev/null
            echo -e "  ${GREEN}✔ DDoS Protection AKTIF!${NC}"; sleep 3 ;;
        2) clear; iptables -L -n -v 2>/dev/null | head -30; echo ""; read -p "  Press any key..." ;;
...
_adv_firewall() {
    ...
    if command -v ufw >/dev/null 2>&1; then
        ...
        echo ""
        echo -e "  ${WHITE}[1]${NC} Enable UFW  ${WHITE}[2]${NC} Disable  ${WHITE}[3]${NC} Allow Port  ${WHITE}[0]${NC} Back"
        read -p "  Select: " c
        case $c in
            1) ufw allow 22/tcp >/dev/null 2>&1; ufw allow 443/tcp >/dev/null 2>&1; echo "y" | ufw enable >/dev/null 2>&1; echo -e "  ${GREEN}✔ UFW Enabled!${NC}"; sleep 2 ;;
            2) ufw disable >/dev/null 2>&1; echo -e "  ${YELLOW}UFW Disabled${NC}"; sleep 2 ;;
            3) read -p "  Port: " port; [[ -n "$port" ]] && ufw allow "$port" >/dev/null 2>&1; sleep 2 ;;
        esac
    else
        read -p "  Install UFW? [y/N]: " c
        [[ "$c" == "y" ]] && apt-get install -y ufw >/dev/null 2>&1 && echo -e "  ${GREEN}✔ UFW terinstall!${NC}"
        sleep 2
    fi
}
...
_adv_user_limits() {
    ...
    if [[ ${#files[@]} -gt 0 ]]; then
        for f in "${files[@]}"; do
            ...
            printf "  ${GREEN}%-20s${NC} %-10s ${YELLOW}%s IP${NC}\n" "$uname" "$proto" "${limit:-N/A}"
        done
    else
        echo -e "  ${RED}Tidak ada akun!${NC}"
    fi
    echo ""
    echo -e "  ${WHITE}[1]${NC} Update limit akun  ${WHITE}[0]${NC} Back"
    read -p "  Select: " c
    case $c in
        1)
...
_adv_custom_payload() {
    clear; print_menu_header "CUSTOM PAYLOAD GENERATOR"
    [[ -f "$DOMAIN_FILE" ]] && DOMAIN=$(tr -d '\n\r' < "$DOMAIN_FILE" | xargs)
    echo ""
    echo -e "  ${WHITE}1. HTTP Upgrade:${NC}"
    printf "  ${GREEN}GET / HTTP/1.1[crlf]Host: %s[crlf]Upgrade: websocket[crlf][crlf]${NC}\n" "$DOMAIN"
    echo ""
    echo -e "  ${WHITE}2. HTTP CONNECT:${NC}"
    printf "  ${GREEN}CONNECT %s:443 HTTP/1.1[crlf]Host: %s[crlf][crlf]${NC}\n" "$DOMAIN" "$DOMAIN"
    echo ""
...
_adv_cron_jobs() {
    ...
    echo ""
    echo -e "  ${WHITE}[1]${NC} Auto hapus expired  ${WHITE}[2]${NC} Auto restart xray  ${WHITE}[3]${NC} Hapus semua  ${WHITE}[0]${NC} Back"
    read -p "  Select: " c
    case $c in
        1) (crontab -l 2>/dev/null; echo "0 0 * * * bash /root/tunnel.sh delete_expired_cron 2>/dev/null") | crontab -; echo -e "  ${GREEN}✔ Done!${NC}"; sleep 2 ;;
        2) (crontab -l 2>/dev/null; echo "0 4 * * * systemctl restart xray >/dev/null 2>&1") | crontab -; echo -e "  ${GREEN}✔ Done!${NC}"; sleep 2 ;;
        3) crontab -r 2>/dev/null; echo -e "  ${YELLOW}Semua dihapus!${NC}"; sleep 2 ;;
    esac
}

_adv_system_logs() {
    clear; print_menu_header "SYSTEM LOGS"
    echo -e "  ${CYAN}[1]${NC} Xray Access  ${CYAN}[2]${NC} Xray Error  ${CYAN}[3]${NC} Nginx  ${CYAN}[4]${NC} SSH Auth  ${CYAN}[5]${NC} System  ${CYAN}[0]${NC} Back"
    read -p "  Select: " log_choice
    clear
    case $log_choice in
        1) tail -50 /var/log/xray/access.log 2>/dev/null || echo "  No logs" ;;
        2) tail -50 /var/log/xray/error.log 2>/dev/null || echo "  No logs" ;;
        3) tail -50 /var/log/nginx/error.log 2>/dev/null || echo "  No logs" ;;
        4) tail -50 /var/log/auth.log 2>/dev/null || echo "  No logs" ;;
        5) journalctl -n 50 --no-pager ;;
    esac
    echo ""; read -p "  Press any key to back..."
}
...
#================================================
# TUNNEL BOT MULTI-VPS (BACKGROUND, SILENT)
#================================================
...
def run_remote(ip, cmd):
    c = f"ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 -o BatchMode=yes root@{ip} '{cmd}'"
    try:
...
def run_on(ip, cmd):
    return run_local(cmd) if ip == get_ip() else run_remote(ip, cmd)
...
def make_links(protocol, username, uid, domain):
    if protocol == "vmess":
        ...
        gj   = json.dumps({"v":"2","ps":username,"add":domain,"port":"8444","id":uid,
        ...
        grpc = f"vless://{uid}@{domain}:8444?mode=gun&security=tls&encryption=none&type=grpc&serviceName=vless-grpc&sni=bug.com#{username}"
    else:
        ...
        grpc = f"trojan://{uid}@{domain}:8444?mode=gun&security=tls&type=grpc&serviceName=trojan-grpc&sni=bug.com#{username}"
...
auto_install() {
    ...
    _run "Downloading & Installing Xray" "bash <(curl -Ls https://github.com/XTLS/Xray-install/raw/main/install-release.sh)"
    mkdir -p "$AKUN_DIR" /var/log/xray /usr/local/etc/xray "$PUBLIC_HTML" "$ORDER_DIR" /root/bot
    command -v xray >/dev/null 2>&1 && _ok "Xray installed" || _fail "Xray install failed"
...
DBEOF
    configure_haproxy
    _ok "Dropbear port 222 & HAProxy port 443"
    ...
    # ── INSTALL TUNNEL BOT MULTI-VPS (SILENT BACKGROUND) ──
