...
# │  Nginx Download : 81                    │
# │  HAProxy TLS    : 443 → proxy ke Xray   │
# │                                         │
# │  Xray VMess WS TLS    : 10443 (via 443) │
# │  Xray VMess WS NonTLS : 10080 (via 80)  │
# │  Xray VLess WS TLS    : 10443 (via 443) │
# │  Xray VLess WS NonTLS : 10080 (via 80)  │
# │  Xray Trojan WS TLS   : 10443 (via 443) │
# │  Xray gRPC TLS        : 10444           │
# │                                         │
# │  BadVPN UDP     : 7100-7300             │
# └─────────────────────────────────────────┘
#
# SOLUSI PORT TABRAKAN:
# - Semua protokol TLS pakai 1 port internal (10443)
#   dibedakan lewat PATH di WebSocket:
#   /vmess → VMess
#   /vless → VLess
#   /trojan → Trojan
# - HAProxy di port 443 sebagai pintu masuk TLS
# - Nginx di port 80 sebagai pintu masuk NonTLS
# - gRPC di port 10444 khusus
#================================================
...
# Port 443 masuk → langsung ke Xray TLS port 10443
# Xray yang akan bedakan VMess/VLess/Trojan via path WebSocket
frontend front_443
    bind *:443
    mode tcp
...
backend back_xray_tls
    mode tcp
    option tcp-check
    server xray_tls 127.0.0.1:10443 check inter 10s rise 2 fall 3 send-proxy-v2
HAEOF
}
...
create_account_template() {
    ...
    # Generate links
    # TLS = via HAProxy port 443
    # NonTLS = via Nginx port 80
    # gRPC = langsung port 10444
...
_print_xray_result() {
    ...
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "  ${YELLOW}[TLS via HAProxy port 443]${NC}"
    ...
    printf "  ${WHITE}%-16s${NC} : %s\n" "SNI"         "$DOMAIN"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "  ${YELLOW}[NonTLS via Nginx port 80]${NC}"
    printf "  ${WHITE}%-16s${NC} : %s\n" "Port"        "80"
    printf "  ${WHITE}%-16s${NC} : %s\n" "Network"     "WebSocket"
    printf "  ${WHITE}%-16s${NC} : %s\n" "Path"        "/${protocol}"
    printf "  ${WHITE}%-16s${NC} : %s\n" "TLS"         "disabled"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "  ${YELLOW}[gRPC TLS port 10444]${NC}"
    printf "  ${WHITE}%-16s${NC} : %s\n" "Port"        "10444"
    printf "  ${WHITE}%-16s${NC} : %s\n" "Network"     "gRPC"
    printf "  ${WHITE}%-16s${NC} : %s\n" "ServiceName" "${protocol}-grpc"
    printf "  ${WHITE}%-16s${NC} : %s\n" "TLS"         "enabled"
...
menu_ssh() {
    while true; do
        ...
        echo -e "  ${WHITE}[5]${NC} List User SSH"
...
menu_vmess() {
    while true; do
        ...
        echo -e "  ${WHITE}[5]${NC} List User VMess"
        echo -e "  ${WHITE}[0]${NC} Back To Menu"
        echo ""
        read -p "  Select: " choice
        case $choice in
...
menu_vless() {
    while true; do
        ...
        echo -e "  ${WHITE}[5]${NC} List User VLess"
        echo -e "  ${WHITE}[0]${NC} Back To Menu"
        echo ""
        read -p "  Select: " choice
        case $choice in
...
menu_trojan() {
    while true; do
        ...
        echo -e "  ${WHITE}[5]${NC} List User Trojan"
        echo -e "  ${WHITE}[0]${NC} Back To Menu"
        echo ""
        read -p "  Select: " choice
        case $choice in
...
menu_uninstall() {
    while true; do
        ...
        echo -e "  ${WHITE}[6]${NC} Uninstall Bot"
        echo -e "  ${RED}[7]${NC} ${RED}HAPUS SEMUA${NC}"
